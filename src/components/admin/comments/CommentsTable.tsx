...
import { AdminComment } from '@/hooks/useAdmin';
...
interface CommentsTableProps {
  ...
  onResetFilters: () => void;
  onUpdateStatus: (commentId: string, status: string) => void;
  onOpenReply: (comment: AdminComment) => void;
  onDelete: (comment: AdminComment) => void;
}

const columnHelper = createColumnHelper<AdminComment>();
...
export default function CommentsTable({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'rejected':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 whitespace-nowrap">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 whitespace-nowrap">
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('userName', {
        ...
        cell: ({ row }) => {
          ...
          const name = c.userName || 'کاربر مهمان';
          const email = c.userEmail || '';
          return (
            <div className="flex items-center gap-2.5 min-w-[140px] max-w-[200px]">
              ...
              <div className="flex flex-col min-w-0">
                ...
                {email && (
                  <span className="text-[10px] text-muted-foreground truncate dir-ltr text-right">
                    {email}
                  </span>
                )}
              </div>
      ...
      columnHelper.accessor('productName', {
        id: 'product',
        header: () => <span className="font-black">محصول</span>,
        cell: ({ row }) => {
          const c = row.original;
          const pName = c.productName || 'محصول آرتیسا';
          return (
            <div className="flex items-center gap-1.5 max-w-[180px]">
              <Link
                ...
                target="_blank"
                rel="noreferrer"
                className="text-primary hover:underline text-xs font-bold truncate"
                title={pName}
              >
                {pName}
              </Link>
      ...
      columnHelper.display({
        id: 'type_rating',
        header: () => <span className="font-black">نوع و امتیاز</span>,
        cell: ({ row }) => {
          const c = row.original;
          const isQuestion = c.type === 'question' || (c.text && c.text.includes('؟'));
          return (
            <div className="flex items-center gap-2 whitespace-nowrap">
              {isQuestion ? (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 font-extrabold text-[10px] border border-indigo-500/20">
                  <HelpCircle className="size-2.5" />
                  پرسش
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-teal-500/10 text-teal-600 dark:text-teal-400 font-extrabold text-[10px] border border-teal-500/20">
              ...
              {c.rating && !isQuestion ? (
                <div className="flex items-center gap-0.5 text-xs font-bold text-foreground">
                  ...
                  <span>{toPersianDigits(c.rating)}</span>
                </div>
              ) : null}
            </div>
          );
      ...
      columnHelper.accessor('text', {
        id: 'text',
        header: () => <span className="font-black">متن پیام و پاسخ</span>,
        cell: ({ row }) => {
          const c = row.original;
          return (
            <div className="flex flex-col gap-1 min-w-[240px] max-w-[380px]">
              <span className="text-xs text-foreground font-semibold line-clamp-2" title={c.text}>
                {c.text}
              </span>
              {c.reply && (
                <div className="flex items-center gap-1.5 text-[10px] text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 p-1.5 rounded-lg border border-emerald-500/20">
                  <Reply className="size-3 shrink-0 rotate-180" />
                  <span className="truncate" title={c.reply}>
                    پاسخ: {c.reply}
                  </span>
                </div>
              )}
    ...
    [onUpdateStatus, onOpenReply, onDelete]
  );
  ...
  const columnLabels: Record<string, string> = {
    user: 'کاربر',
    product: 'محصول',
    type_rating: 'نوع و امتیاز',
    text: 'متن پیام و پاسخ',
    date: 'تاریخ',
    status: 'وضعیت',
  };
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 bg-background/95 border border-border/60 p-4 rounded-3xl backdrop-blur-xl shadow-xs">
        <div className="flex flex-col sm:flex-row items-center gap-3 flex-1">
          ...
          >
            ...
            <option value="question">پرسش‌ها</option>
          </select>
        </div>
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2 shrink-0">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-3 h-10 rounded-2xl bg-primary/10 border border-primary/20 text-primary text-xs font-extrabold">
            ...
            <span>
              {toPersianDigits(totalCount.toLocaleString('fa-IR'))} پیام
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-border/60 bg-background/95 backdrop-blur-xl shadow-sm min-w-0 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-16 text-center flex flex-col items-center justify-center gap-3">
            ...
            <h3 className="text-sm font-black text-foreground mt-1">هیچ دیدگاهی یافت نشد</h3>
            <p className="text-xs font-medium text-muted-foreground max-w-sm">
              دیدگاهی با فیلترهای جستجوی فعلی مطابقت ندارد. می‌توانید فیلترها را تغییر داده یا بازنشانی کنید.
            </p>
            {hasActiveFilters && (
              <Button
                variant="outline"
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-border/50 bg-muted/15 text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
              ...
              <span className="text-[11px] font-bold">
                ...
                از <strong className="text-foreground">{toPersianDigits(totalCount)}</strong> پیام
              </span>
            </div>
